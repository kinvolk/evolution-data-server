                                ...
                                GParamSpec *pspec)
{
	switch (property_id) {
		...
		case PROP_TEMPLATE:
			g_value_take_string (
				value,
				e_source_autoconfig_dup_template (
				E_SOURCE_AUTOCONFIG (object)));
			return;
	}
...
e_source_autoconfig_class_init (ESourceAutoconfigClass *class)
{
	...
	g_object_class_install_property (
		...
		g_param_spec_string (
			"template",
			"Template",
			"Identifier to map user-specific sources to system-wide sources",
			"",
			G_PARAM_READWRITE |
			G_PARAM_CONSTRUCT |
			G_PARAM_STATIC_STRINGS |
...
/**
 * e_source_autoconfig_get_template:
 * @extension: an #ESourceAutoconfig
 *
 * Returns the template of a data source. This maps user-specific sources to
 * system-wide sources.
 *
 * If they match, the revision field will be checked.
 *
 * Returns: template of the data source
 *
 * Since: 3.24
 **/
const gchar *
e_source_autoconfig_get_template (ESourceAutoconfig *extension)
{
	g_return_val_if_fail (E_IS_SOURCE_AUTOCONFIG (extension), NULL);

	return extension->priv->template;
}
 ...
 * @extension: an #ESourceAutoconfig
 *
 * Thread-safe variation of e_source_autoconfig_get_template().
 * Use this function when accessing @extension from multiple threads.
 *
 * The returned string should be freed with g_free() when no longer needed.
 *
 * Returns: a newly-allocated copy of #ESourceAutoconfig:template
 *
 * Since: 3.24
 **/
gchar *
e_source_autoconfig_dup_template (ESourceAutoconfig *extension)
{
	...
	protected = e_source_autoconfig_get_template (extension);
 ...
 *
 * Sets the template used to map user-specific sources to system-wide ones.
 *
 * If it matches, the revision field will be checked.
 *
 * The internal copy of @template is automatically stripped of leading and
 * trailing whitespace.
 *
 * Since: 3.24
 **/
void
e_source_autoconfig_set_template (ESourceAutoconfig *extension,
                                  const gchar *template)
{
	...
	if (g_strcmp0 (extension->priv->template, template) == 0) {
		e_source_extension_property_unlock (E_SOURCE_EXTENSION (extension));
		return;
	}

	g_free (extension->priv->template);
	extension->priv->template = e_util_strdup_strip (template);

	e_source_extension_property_unlock (E_SOURCE_EXTENSION (extension));

	g_object_notify (G_OBJECT (extension), "template");
}
